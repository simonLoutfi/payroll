...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO motherhood (motherhood_basis, motherhood_company, motherhood_staff) VALUES (:basis, :company, :staff)";
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  <div id="iframeContainer"></div>
    ...
    <div class="pagetitle">
      <h1>Motherhood and Illness Subscription</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Parameters</li>
          <li class="breadcrumb-item active">Motherhood and Illness Subscription</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->

    <section class="section">
    <form action="motherhood.php" method="post" id='myForm'>
    <div>
        ...
        try {
            $sql = "SELECT MAX(changeDate) AS max_date FROM motherhood";   
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
    ...
    <div>
        <div style='margin-top:1%'>
            <label for='basis' style='margin-left:1%;'>Basis:</label>
            <input id='basis' name='basis' type='number' value='' style='margin-left:1%;'/>
        </div>
        <div style='margin-top:1%'>
            <label for='company' style='margin-left:1%;'>Company:</label>
            <input id='company' name='company' type='number' value='' style='margin-left:1%;'/>
        </div>
        <div style='margin-top:1%'>
            <label for='staff' style='margin-left:1%;'>Staff:</label>
            <input id='staff' name='staff' type='number' value='' style='margin-left:1%;'/>
        </div>
    </div>
    <br/>
    ...
    function checkDisplay(obj) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                document.getElementById('staff').value = response['staff'];
            }
        };
        ...
        xhr.open("GET", "motherhoodValues.php?date=" + encodeURIComponent(formattedDate), true);
        xhr.send();
    }
