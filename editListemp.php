...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['salaries'])) {
    try {
        foreach ($_POST['salaries'] as $emp_id => $salary_data) {
            foreach ($salary_data as $index => $amount) {
                $stmt = $pdo->prepare("UPDATE salary SET amount = :amount WHERE emp_id = :emp_id AND salCat_id = :salCat_id");
                $stmtHist = $pdo->prepare("INSERT INTO salary_history (emp_id, salCat_id, amount) VALUES (:emp, :salCat, :amt)");
                
                $category_name = $_POST['salCatname'][$index];
                $stmt_cat_id = $pdo->prepare("SELECT salCat_id FROM `salary category` WHERE salCat_name = :category_name");
                $stmt_cat_id->bindParam(':category_name', $category_name);
                $stmt_cat_id->execute();
                $salCat_id = $stmt_cat_id->fetchColumn();

                $stmt->bindParam(':emp_id', $emp_id);
                $stmt->bindParam(':salCat_id', $salCat_id);
                $stmt->bindParam(':amount', $amount);
                $stmtHist->bindParam(':emp', $emp_id);
                $stmtHist->bindParam(':salCat', $salCat_id);
...
<head>
  ...
  <title>Edit Employee by List</title>
...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <div id="iframeContainer"></div>
    ...
    <div class="pagetitle">
      <h1>Edit Employee By List</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Employee</li>
          <li class="breadcrumb-item active">Edit Employee By List</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
<script>
...
include 'connection.php'; 
?>
<form action='editListemp.php' method='post' id='myForm'>
<table class='table' id='salary-table'>
    <tr>
        <th>ID</th>
        <th>Name</th>
        <?php
        $stmt_categories = $pdo->prepare("SELECT salCat_name FROM `salary category` sc INNER JOIN `salary category type` sct ON sc.salCatType_id=sct.salCatType_id
    ...
    <?php
    try {
        $stmt = $pdo->prepare("SELECT e.emp_id, concat_fname_lname AS full_name, 
                                GROUP_CONCAT(amount ORDER BY salCat_name) AS amounts, 
                                GROUP_CONCAT(salCat_name ORDER BY salCat_name) AS categories
                                FROM employee e 
                                INNER JOIN salary s ON e.emp_id=s.emp_id
                                INNER JOIN `salary category` sc ON s.salCat_id=sc.salCat_id
        ...
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            $amounts = explode(',', $row["amounts"]);
                foreach ($categories as $index => $category) {
                    echo "<td><input type='text' name='salaries[" . $row["emp_id"] . "][$index]' class='editable' value='" . $amounts[$index] . "'></td>";
                    echo "<input type='hidden' name='salCatname[$index]' value='" . $category . "'>";
