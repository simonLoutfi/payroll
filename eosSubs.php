...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO eos_subscription (eosSubs_compContr, eosSubs_compSur) VALUES (:basis, :percentage)";
...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <div id="iframeContainer"></div>
    ...
    <div class="pagetitle">
      <h1>End Of Service Indemnity Subscription</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Parameters</li>
          <li class="breadcrumb-item active">End Of Service Indemnity Subscription</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->

    <section class="section">
    <form action="eosSubs.php" method="post" id='myForm'>
    <div>
        ...
        try {
            $sql = "SELECT MAX(changeDate) AS max_date FROM eos_subscription";   
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
    ...
    <div>
        <div style='margin-top:1%'>
            <label for='basis' style='margin-left:1%;'>Company Contribution:</label>
            <input id='basis' name='basis' type='number' value='' style='margin-left:1%;'/>
        </div>
        <div style='margin-top:1%'>
            <label for='perc' style='margin-left:1%;'>Company Surcharge:</label>
...
<script>
    function checkDisplay(obj) {
        ...
        xhr.open("GET", "eosValues.php?date=" + encodeURIComponent(formattedDate), true);
        xhr.send();
    }
