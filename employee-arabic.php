...
<head>
  ...
  <title>Arabic information</title>
...
<li class="nav-item">
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Arabic Information</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Employee</li>
          <li class="breadcrumb-item active">Arabic Information</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section">
      <div id="arabic-form">
        ...
        <form action="arabic_info_ctrl.php" method='post'>
          <div>
            <label for="firstName">اسم المستخدم</label>
            <input id="firstName" name="firstName" class="form-control" dir="rtl" oninput="validateArabicInput(this)"
              required style="margin-right : right;" />
            ...
            <script>
              function validateArabicInput(input) {
                ...
                if (!arabicPattern.test(input.value)) {
                  input.value = input.value.replace(/[^\u0600-\u06FF\s]/g, ''); // Remove non-Arabic characters
                }
              }
            </script>

            <label for="father">اسم الأب</label>
            <input id="father" name="father" class="form-control" oninput="validateArabicInput(this)" required />

            <label for="lastName">اسم الشهرة</label>
            <input id="lastName" name="lastName" class="form-control" oninput="validateArabicInput(this)"
              required /><br />

            <label for="NssfNum">رقم التسجيل الشخصي</label>
            <input type="number" id="NssfNum" name="NssfNum" class="form-control" required />

            <label for="jobDesc">نوع العمل</label>
            <input id="jobDesc" name="jobDesc" class="form-control" oninput="validateArabicInput(this)"
              required /><br />

            <label for="status">الوضع العائلي</label>
            ...
            <input type="number" id="children" name="children" class="form-control" min="0" max="5" required />

            <label for="workType">نوع الأجر</label>
            ...
            <label for="workHour">عدد ساعات العمل في الشهر</label>
            <input type="number" id="workHour" name="workHour" class="form-control" min="0" required />

            <label for="paymentType">طريقة دفع الاجر</label>
            <select id="paymentType" name="paymentType" class="form-control" required>
              <option value="" disabled selected></option>
              <option value="على الانتاج">على الانتاج</option>
              <option value="لقاء عمولة">لقاء عمولة</option>
              <option value="يومي">يومي</option>
              <option value="اسبوعي">اسبوعي</option>
              <option value="شهري">شهري</option>
            </select>

            <label for="LeavingReason"> سبب ترك العمل</label>
            <select id="LeavingReason" name="LeavingReason" class="form-control" required>
              <option value="" disabled selected></option>
              <option value="لا شيء"> لا شيء</option>
              <option value="عمل آخر"> عمل آخر </option>
              <option value="هجرة"> هجرة </option>
              <option value="وفاة"> وفاة </option>
              <option value="زواج"> زواج </option>
              <option value="عجز"> عجز </option>
              <option value="بلوغ السن"> بلوغ السن </option>
              <option value="استقالة"> استقالة </option>
            </select> <br />

            <p><b><u>عنوان المستخدم/الاجير</u></b></p></br>

            <label for="region"> منطقة </label>
            <input id="region" name="region" class="form-control" required/>
              


            <label for="city"> بلدة</label>
            <input id="city" name="city" class="form-control" required/>

            <label for="mohafaza">محافظة</label>
            <input id="mohafaza" name="mohafaza" class="form-control" required/>
           
            <label for="casa">قضاء</label>
            <input id="casa" name="casa" class="form-control" required/>
              </br>


            <label for="District">الحي</label>
            <input id="District" name="District" class="form-control" oninput="validateArabicInput(this)" required />

            <label for="street">الشارع</label>
            <input id="street" name="street" class="form-control" style=" width:40% ; size:100 "
              oninput="validateArabicInput(this)" required /></br>

            <label for="building">مبنى</label>
            <input id="building" name="building" class="form-control" oninput="validateArabicInput(this)" required />

            <label for="floor">الطابق</label>
            <input id="floor" name="floor" class="form-control" oninput="validateArabicInput(this)" required />
            ...
            <input type="email" id="email" name="email" class="form-control" style=" width:40% ; size:100 " required />
          ...
          <div>
            ...
            <!-- <a href="#"><button type="button" class="btn btn-primary"><i class="bi bi-arrow-return-left"></i> معلومات إضافية</button></a> -->
          </div>
        </form>
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if (response.error) {
          ...
          setSelectedValue(document.getElementById("mohafaza"), response.mohafaza_name);
        }
      }
    };
