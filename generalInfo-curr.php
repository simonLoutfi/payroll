...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $cur_reference = isset($_POST['cur_reference']) ? $_POST['cur_reference'] : null;
  $rate = $_POST['rate'];

  try {
      if ($cur_reference !== null) {
          // Check if the combination already exists in the database
          $checkQuery = "SELECT cur_id FROM currency WHERE (cur_name = :cur_name AND cur_reference = :cur_reference) OR (cur_name = :cur_reference AND cur_reference = :cur_name)";
          $stmtCheck = $pdo->prepare($checkQuery);
          $stmtCheck->execute(['cur_name' => $cur_name, 'cur_reference' => $cur_reference]);
          $existingCurrency = $stmtCheck->fetch(PDO::FETCH_ASSOC);

          if ($existingCurrency) {
              // If the combination exists, use the existing cur_id
              $cur_id = $existingCurrency['cur_id'];
          } else {
              // If the combination does not exist, insert into the currency table
              $insertCurrencyQuery = "INSERT INTO currency (cur_name, cur_reference) VALUES (:cur_name, :cur_reference)";
              $stmtInsertCurrency = $pdo->prepare($insertCurrencyQuery);
              $stmtInsertCurrency->execute([':cur_name' => $cur_name, ':cur_reference' => $cur_reference]);

              // Fetch the cur_id of the inserted currency
              $cur_id = $pdo->lastInsertId();
          }

          // Insert cur_id, rate, and current date into the rate table
          $insertRateQuery = "INSERT INTO rate (cur_id, rate, rate_date) VALUES (:cur_id, :rate, NOW())";
          $stmtInsertRate = $pdo->prepare($insertRateQuery);
          $stmtInsertRate->execute([':cur_id' => $cur_id, ':rate' => $rate]);
      } else {
          echo "Error: 'cur_reference' cannot be null.";
      }
  ...
  <li class="nav-item">
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
         ...
         <form method="POST" action="generalInfo-curr.php" id='dataForm'>
            <div class="row mb-3">
                <label for="inputText" class="col-sm-1 col-form-label">Currency:</label>
                <div class="col-sm-2 ms-1">
                    <input type="text" class="form-control" id="inputCurrency" name="cur_name">
                </div>
            </div>
            <div class="row mb-3">
                <label for="inputEmail" class="col-sm-1 col-form-label ">Reference:</label>
                <div class="col-sm-2 ms-2">
                    <input type="text" class="form-control" id="inputRefrence" name="cur_reference">
                </div>
            </div>
            <div class="row mb-3">
                <label for="inputPassword" class="col-sm-1 col-form-label ">Rate:</label>
                <div class="col-sm-2">
                    <input type="number" class="form-control" id="inputPassword" name="rate">
                </div>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id='dataForm'>
                    <div class="mb-3">
                        <label for="select1" class="form-label">Currency</label>
                        ...
                        <?php
                          ...
                          try {
                              $sql = "SELECT c.*, r.rate, r.rate_date
                                      FROM currency c
                                      INNER JOIN (
                                          SELECT cur_id, MAX(rate_date) AS max_rate_date
                                          FROM rate
                                          GROUP BY cur_id
                                      ) latest_rates ON c.cur_id = latest_rates.cur_id
                                      INNER JOIN rate r ON latest_rates.cur_id = r.cur_id AND latest_rates.max_rate_date = r.rate_date";
                              $stmt = $pdo->query($sql);

                              if ($stmt->rowCount() > 0) {
                                  while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                      echo "<option value='" . $row["cur_id"] . "' data-reference='" . $row["cur_reference"] . "' data-rate='" . $row["rate"] . "'>" . $row["cur_name"] . "</option>";
                                  }
                              }
                          } catch(PDOException $e) {
                    ...
                    <div class="mb-3">
                        ...
                        <!-- Hidden input field to store the rate value -->
...
<script>
    $(document).ready(function(){
        // Function to update the reference and rate inputs based on the selected value of select1
        $("#select1").change(function(){
            var curReference = $(this).find(":selected").data("reference");
            var curRate = $(this).find(":selected").data("rate");
            $("#reference").val(curReference);
            $("#rate").val(curRate);
            // Update the hidden input field with the rate value
            $("#hiddenRate").val(curRate);
        });

        // AJAX request to insert data into the database when submit button is clicked
        $("#submitButton").click(function(){
            var formData = {
                currencyId: $("#select1").val(),
    ...
    error: function(xhr, status, error){
        ...
        console.error("Error:", xhr.responseText);
            ...
            // Console log formData to check its contents
    ...
    <script>
      $(document).ready(function() {
        $('#inputCurrency').autocomplete({
          source: function(request, response) {
            $.ajax({
              url: "autocomplete_curr.php",
              type: "GET",
              dataType: "json",
              data: {
                action: 'currencyNames', // Specify the action for currency names
                term: request.term
              },
              success: function(data) {
                console.log("Received currency names data:", data);
                response(data);
              },
              error: function(xhr, status, error) {
        ...
        $('#inputRefrence').autocomplete({
          source: function(request, response) {
            $.ajax({
              url: "autocomplete_curr_ref.php",
              type: "GET",
              dataType: "json",
              data: {
                action: 'currencyReferences', // Specify the action for currency references
                term: request.term
              },
              success: function(data) {
                console.log("Received reference data:", data);
                response(data);
              },
              error: function(xhr, status, error) {
