...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    include 'connection.php'; // Assuming connection.php is the file for database connection

    $branch_name = $_POST['branch_name'];

    try {
        // Check if the branch already exists
        $check_branch_query = "SELECT * FROM branch WHERE branch_name = :branch_name";
        $check_branch_stmt = $pdo->prepare($check_branch_query);
        $check_branch_stmt->bindParam(':branch_name', $branch_name);
        $check_branch_stmt->execute();

        if($check_branch_stmt->rowCount() > 0) {
            echo "<script type='text/javascript'>alert('Branch already exists.');</script>";
        } else {
            // Insert the new branch
            $insert_branch_query = "INSERT INTO branch (branch_name) VALUES (:branch_name)";
            $insert_branch_stmt = $pdo->prepare($insert_branch_query);
            $insert_branch_stmt->bindParam(':branch_name', $branch_name);
            if(!$insert_branch_stmt->execute()) {
              echo "<script type='text/javascript'>alert('Error adding branch: ".$pdo->errorInfo()[2]."');</script>";
            } 
        }
    } catch(PDOException $e) {
        echo "<script type='text/javascript'>alert('PDO Error: ".$e->getMessage()."');</script>";
    }
    $pdo = null;
}
?>
...
// document.addEventListener('DOMContentLoaded', function() {
//   var openIframeLink = document.getElementById('openIframeLink');
//   openIframeLink.addEventListener('click', function(event) {
//     event.preventDefault(); // Prevent default link behavior
//     var iframeContainer = document.getElementById('iframeContainer');
//     iframeContainer.innerHTML = '<iframe src="employee-add.php" width="100%" height="600" frameborder="0" scrolling="auto"></iframe>';
//   });
// });
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <ul id="icons-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
          ...
          <li>
            <a href="#">
              <i class="bi bi-circle"></i><span id='openIframeLink'>r7</span>
            </a>
          </li>
        </ul>
      </li>
  ...
  <div id="iframeContainer"></div>
    ...
    <div id="iframeContainer" class="mt-4">  </div>
      <div>
          <form action="generalInfo-branch.php" method="post" id='myForm'>
              <label for="branch_name">Branch Name:</label>
              <input type="text" id="branch_name" name="branch_name" class="form-control" required><br>
              <button type="submit"  class="btn btn-primary"><i class="bi bi-save-fill"></i> Add Branch</button>
          </form>
      </div>
    </section>
