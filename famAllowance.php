<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        try {
            foreach ($_POST['member'] as $memberType_id => $amount) {
                $insert_sql = "INSERT INTO family_allowances (memberType_id, amount) VALUES (:memberType_id, :amount)";
                $insert_stmt = $pdo->prepare($insert_sql);
                $insert_stmt->bindParam(':memberType_id', $memberType_id);
...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <div id="iframeContainer"></div>
    ...
    <div class="pagetitle">
      <h1>Family Allowances</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item">Parameters</li>
          <li class="breadcrumb-item active">Family Allowances</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->

    <section class="section">
        <form action="famAllowance.php" method="post" id='myForm'>
        <script>
          function checkDisplay(obj) {
              ...
              xhr.onreadystatechange = function() {
                  if (xhr.readyState == 4 && xhr.status == 200) {
                      ...
                      // Loop through the response and set values to input fields
                      for (var memberId in response) {
                          var amountInput = document.getElementById(memberId);
                          if (amountInput) {
                              amountInput.value = response[memberId];
                          }
                      }
                  }
              };
              xhr.open("GET", "get_values.php?date=" + selectedDate, true);
              xhr.send();
          }
            ...
            <div>
                <?php
                    ...
                    try {
                        ...
                        $sql = "SELECT MAX(changeDate) AS max_date FROM family_allowances";
                        ...
                        echo "<input id='date' name='date' type='date' style='margin-left:1%;' onchange='checkDisplay(this)' value='" . $maxDate . "' />";

                      $dateVal = isset($_POST['dateVal']) ? $_POST['dateVal'] : $maxDate; 
                      ...
                      if ($stmt->rowCount() > 0) {
                          while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                              $amount_sql = "SELECT amount FROM family_allowances WHERE memberType_id = :memberType_id AND changeDate = :changeDate";
                              $amount_stmt = $pdo->prepare($amount_sql);
                              $amount_stmt->bindParam(':memberType_id', $row["memberType_id"]);
                              $amount_stmt->bindParam(':changeDate', $dateVal);
                              $amount_stmt->execute();
                              $amount_row = $amount_stmt->fetch(PDO::FETCH_ASSOC);

                              $amount = ($amount_row) ? $amount_row['amount'] : ''; 

                              echo "<div style='margin-top:1%'><label for='" . $row["memberType_id"] . "' style='margin-left:1%;'>";
                              echo $row["memberType_desc"]; 
                              echo "</label>";
                              echo "<input id='" . $row["memberType_id"] . "' name='member[" . $row["memberType_id"] . "]' type='number' value='" . $amount . "' style='margin-left:1%;'/></div><br/>";
                          }
                      }
                  } catch (PDOException $e) {
                      echo "PDO Error: " . $e->getMessage();
