<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $selectedSalCatTypeId = $_POST['radiogroup'];

      require 'connection.php';

      try {
          // Check if already exist
          $checkQuery = "SELECT * FROM `salary category` WHERE salCat_name = :field AND salCatType_id = :radiogroup";
          $checkStmt = $pdo->prepare($checkQuery);
          $checkStmt->bindParam(':field', $fieldName);
          $checkStmt->bindParam(':radiogroup', $selectedSalCatTypeId);
          $checkStmt->execute();

          if ($checkStmt->rowCount() > 0) {
            echo "<script type='text/javascript'>alert('Info added previously');</script>";
          } else {
              $insertQuery = "INSERT INTO `salary category` (salCat_name, salCatType_id) VALUES (:field, :radiogroup)";
              $insertStmt = $pdo->prepare($insertQuery);
              $insertStmt->bindParam(':field', $fieldName);
              $insertStmt->bindParam(':radiogroup', $selectedSalCatTypeId);
              $insertStmt->execute();
...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <div id="iframeContainer"></div>
    ...
    <section class="section">
      <div>
          <form action="generalInfo-info.php" method="post" id='myForm'>
              <label for="field">Field Name:</label>
              <input type="text" id="field" name="field" class="form-control" required><br>
              <div class="input-group" style="display:block">
                <?php
                  require 'connection.php';

                  try {
                      $sql = "SELECT salCatType_id, salCatType_desc FROM `salary category type`"; // Fixed table name syntax
                      $stmt = $pdo->query($sql);

                      if ($stmt->rowCount() > 0) {
                          while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                              echo "<input type='radio' id='" . $row["salCatType_id"] . "' name='radiogroup' value='" . $row["salCatType_id"] . "'>";
                              echo "<label for='" . $row["salCatType_id"] . "' style='padding-left:1%'>" . $row["salCatType_desc"] . "</label><br>"; // Added semicolon
                          }
                      }
                  } catch(PDOException $e) {
                      echo "PDO Error: " . $e->getMessage();
              ...
              <button type="submit"  class="btn btn-primary" style="margin-top:1%;"><i class="bi bi-save-fill"></i> Add Field</button>
          </form>
      </div>
      <div style="margin-top:3%;">
          <?php
            ...
            $sql = "SELECT salCatType_desc, salCat_name 
                    FROM `salary category type`
                    INNER JOIN `salary category` ON `salary category type`.salCatType_id =`salary category`.salCatType_id";
            ...
            try {
                ...
                if ($stmt->rowCount() > 0) {
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr>";
                        echo "<td>".$row["salCatType_desc"]."</td>";
                        echo "<td>".$row["salCat_name"]."</td>";
                        echo "</tr>";
                    }
                } else {
